...
| Area | Change | Files |
|---|---|---|
| **CRO · critical** | **Express checkout on the product page, cart drawer and cart page.** Shop Pay / PayPal / Apple Pay / Google Pay only appeared on Shopify's checkout step before. PDP uses `{{ form \| payment_button }}` (toggle: Product page → *Show express checkout buttons*); cart surfaces use `content_for_additional_checkout_buttons`. Shop Pay Installments banner (`payment_terms`) above Add to cart. **Gated** (Codex review): dynamic checkout skips the theme's submit handlers and the cart, so the PDP block is rendered hidden and `global.js` reveals it only when no gift-packaging add-on is checked, no validator has vetoed it (the set template's per-piece mode publishes `data-express-blocked` on the form and fires `oc:express-recheck`), and every required engraving field is filled — otherwise a one-line note says why. On `/cart` **and in the drawer** the gift note is persisted to `/cart/update.js` as the shopper types (debounced) and on change/blur through one serialized save queue (stale completions can't release the wallets), and the wallet block is held `inert` + `aria-busy` (with a keydown fallback) until the cart holds the current value — mouse, touch and keyboard. The free-shipping nudge is recomputed in `updateVariant()` for variants with different prices. Contract tests: `tests/personalization/express-checkout.test.mjs`. | `sections/main-product.liquid`, `sections/cart-drawer.liquid`, `sections/main-cart.liquid`, `assets/global.js` |
