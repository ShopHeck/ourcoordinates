...
Theme implementation for priorities 1 and 2 of the storefront audit. This is a development-theme preview and draft PR, not a live release or completion of the entire improvement program. No production theme was published, no checkout submitted, and no Shopify product/page/policy content was mutated.
...
- 92 automated tests pass using the same test glob as CI.
...
Before applying: obtain separate approval for the 12 product-description changes and policy/page cleanup, refetch each exact target, reject timestamp/hash drift, save a fresh rollback backup, whitelist only the approved content fields, and verify the public result. Preserve prices, variants, inventory, media, handles, SEO and product status. Review the refund policy's obsolete proof dependency and the original Shipping/FAQ bodies too, including content surfaced outside the theme. Do not merge this draft as “all delivery conflicts fixed” while those sources remain stale.
...
1. Approve and apply source-content corrections; review this draft, run final CI and merge only after separate release approval. Verify active-theme/public behavior after sync; retain the previous theme for rollback.
